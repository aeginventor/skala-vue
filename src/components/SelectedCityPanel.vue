 ...
 * 카드를 클릭해도 상세 페이지로 바로 넘어가지 않고 "선택"만 되며, 상세 페이지로
 * 가는 길은 이 패널의 "상세보기" 버튼 하나뿐이다 — 클릭 즉시 화면이 바뀌면 방금
 * 선택했다는 반응 자체를 볼 틈이 없기 때문이다.
 * 목록 옆 사이드 패널 대신 화면 하단에 position: fixed로 띄운 이유도 비슷하다.
 * 세로로 긴 사이드 패널은 카드 그리드와 나란히 두면 레이아웃 균형이 깨지고,
