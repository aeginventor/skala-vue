...
/**
 * TempBoard — 30개 지역 기온을 6개씩 나눠 자동으로 넘겨 보여주는 판.
 *
 * 아래 카드 목록은 한 도시당 정보가 많아 30개를 훑으려면 한참 스크롤해야 한다. 여기서는
 * 이름과 기온만 남겨서, 가만히 두면 다섯 장이 돌아가며 전국이 한 바퀴 지나간다.
 * 기온 색은 tempLevel.js의 5단계를 그대로 쓴다 — 카드 배지와 같은 기준이라 두 곳이
 * 어긋나지 않고, 여기가 그 색 램프가 가장 크게 보이는 자리다.
 *
 * 검색 결과와 무관하게 **항상 30개 전체**를 돈다. "전국을 훑는" 판이라서, 검색으로
 * 좁혀진 목록을 따라가면 이 위젯의 목적이 사라진다.
 */
const PAGE_SIZE = 6
