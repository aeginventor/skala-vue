/**
 * 전국 30개 지역 예시(Mock) 데이터.
 *
 * 두 가지 역할을 한다.
 * 1) API 키가 없거나 호출이 실패했을 때의 대체 데이터
 * 2) 헤더의 "예시" 토글을 켰을 때 보여주는 데이터
 *
 * 2번 때문에 **구현해 둔 표현이 전부 한 번씩은 나오도록** 값을 일부러 배치했다.
 * 실제 날씨는 계절을 타서(8월에 눈·뇌우를 볼 수 없다) 눈으로 확인할 방법이 없기 때문이다.
 * - 날씨 그림 6종: 맑음 / 구름·흐림 / 안개 / 비 / 눈 / 뇌우
 * - 구름 개수 3구간: 구름 양 40% 미만(1개) · 40~74%(2개) · 75% 이상(3개)
 * - 온도 5단계: 추움 / 쌀쌀 / 선선 / 따뜻 / 더움
 * 그래서 계절감은 일부러 맞추지 않았다(같은 날 눈과 폭염이 함께 나온다).
 *
 * 조회는 도시명이 아니라 위경도(lat/lon)로 한다. 국내 중소도시는 영문 표기 매칭이
 * 불안정한데(중복 지명, 표기 차이) 좌표는 한 지점만 가리켜 정확하다.
 * region은 Dock/상세 페이지에 노출되는 "행정구역 전체 이름"이자 지역 단위 검색어다.
 */
